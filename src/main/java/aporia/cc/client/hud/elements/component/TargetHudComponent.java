...
import aporia.cc.utility.mixin.accessors.DrawContextAccessor;
    ...
    private void drawArmor(CustomDrawContext ctx, PlayerEntity player, float posX, float posY, float headSize, float padding, float fontSize) {
        ...
        for (ItemStack stack : items) {
            ...
            if (!stack.isEmpty()) {
                ...
                ctx.drawText(xFont, "M", iconX + (boxSizeItem-xFont.width("X"))/2, iconY + (boxSizeItem-xFont.height())/2, Aporia.getInstance().getThemeManager().getCurrentTheme().getGrayLight());
            }
