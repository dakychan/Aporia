...
import aporia.cc.client.screens.autobuy.items.AutoInventoryItem;
...
import static aporia.cc.client.screens.autobuy.items.AutoInventoryBuyScreen.createItem;
...
public final class AutoSbor extends Module {
    ...
    private ArrayList<ItemBuy> getLib(){
        ArrayList<ItemBuy> allItemBuys = new ArrayList<>(funtimeMode.isSelected()?Aporia.getInstance().getAutoBuyManager().getFuntime():Aporia.getInstance().getAutoBuyManager().getHollyworld());
        allItemBuys.addAll(Aporia.getInstance().getAutoBuyManager().getVanilla());
        return allItemBuys;
    }
    @EventTarget
