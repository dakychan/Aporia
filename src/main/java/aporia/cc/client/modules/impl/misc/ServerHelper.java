...
import aporia.cc.base.events.impl.render.EventRender2D;
...
import aporia.cc.client.modules.impl.render.Predictions;
...
import aporia.cc.utility.math.ProjectionUtil;
import aporia.cc.utility.other.BooleanSettable;
...
public final class ServerHelper extends Module {
    ...
    private final BooleanSetting consumablesSetting = new BooleanSetting("Таймер расходников", true,()->Aporia.getInstance().getServerHandler().isCopyTime()||Aporia.getInstance().getServerHandler().isHolyWorld());

    private final BooleanSetting autoPointSetting = new BooleanSetting("Авто точка", true, Aporia.getInstance().getServerHandler()::isFunTime);
    ...
    public void initialize() {
        ...
        keyBindings.add(new KeyBind(Items.FIREWORK_STAR,
                new KeySetting("Анти флай", Aporia.getInstance().getServerHandler()::isReallyWorld),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.FLOWER_BANNER_PATTERN,
                new KeySetting("Опыт прокрутки", Aporia.getInstance().getServerHandler()::isReallyWorld),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.PRISMARINE_SHARD,
                new KeySetting("Взрывная трапка", Aporia.getInstance().getServerHandler()::isHolyWorld),
                5, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.POPPED_CHORUS_FRUIT,
                new KeySetting("Обыч трапка", Aporia.getInstance().getServerHandler()::isHolyWorld),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.NETHER_STAR,
                new KeySetting("Стан", Aporia.getInstance().getServerHandler()::isHolyWorld),
                30, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.FIRE_CHARGE,
                new KeySetting("Взрывная штука", Aporia.getInstance().getServerHandler()::isHolyWorld),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.SNOWBALL,
                new KeySetting("Снежок", () -> Aporia.getInstance().getServerHandler().isCopyTime()
                        || Aporia.getInstance().getServerHandler().isHolyWorld()),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.PHANTOM_MEMBRANE,
                new KeySetting("Божья аура", Aporia.getInstance().getServerHandler()::isCopyTime),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.NETHERITE_SCRAP,
                new KeySetting("Трапка", Aporia.getInstance().getServerHandler()::isCopyTime),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.DRIED_KELP,
                new KeySetting("Пласт", Aporia.getInstance().getServerHandler()::isCopyTime),
                0, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.SUGAR,
                new KeySetting("Явная пыль", Aporia.getInstance().getServerHandler()::isCopyTime),
                10, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.FIRE_CHARGE,
                new KeySetting("Огненный смерч", Aporia.getInstance().getServerHandler()::isCopyTime),
                10, new BooleanSettable()));

        keyBindings.add(new KeyBind(Items.ENDER_EYE,
                new KeySetting("Дезорент", Aporia.getInstance().getServerHandler()::isCopyTime),
                10, new BooleanSettable()));
    ...
    public void onPacket(EventPacket e) {
        ...
        if(this.consumablesSetting.isEnabled()){
            if ( Aporia.getInstance().getServerHandler().isCopyTime()&&e.getPacket() instanceof ChunkDeltaUpdateS2CPacket chunkDelta) {
            ...
            if (e.getPacket() instanceof PlaySoundS2CPacket soundS2CPacket) {
                if (Aporia.getInstance().getServerHandler().isHolyWorld() && soundS2CPacket.getSound().toString().equals("Reference{ResourceKey[minecraft:sound_event / minecraft:block.beacon.deactivate]=SoundEvent[location=minecraft:block.beacon.deactivate, fixedRange=Optional.empty]}")) {
            ...
            if (Aporia.getInstance().getServerHandler().isHolyWorld() && e.getPacket() instanceof ParticleS2CPacket particleS2CPacket) {
        ...
        keyBindings.stream().filter(bind -> bind.draw.isValue()).forEach(bind -> {
            ...
            switch (bind.setting.getName()) {
                case "Трапка", "Обыч трапка" ->
                        drawItemCube(playerPos, smooth, 1.99F, Aporia.getInstance().getThemeManager().getClientColor(90).getRGB());
                case "Дезорент", "Огненный смерч", "Явная пыль" ->
                        drawItemRadius(matrix, bind.distance, ColorUtil.LIGHT_RED);
                case "Взрывная штука" ->
                        drawItemRadius(matrix, 5, Aporia.getInstance().getThemeManager().getClientColor(90).getRGB());
                case "Пласт" -> {
                    ...
                    if (Math.abs(mc.player.getPitch()) > 60) {
                        ...
                        Render3DUtil.drawBox(new Box(pos1, pos2), Aporia.getInstance().getThemeManager().getClientColor(90).getRGB(), 3, true, true, true);
                        ...
                        drawSidePlast(playerPos.east(5).south().down(), smooth, Aporia.getInstance().getThemeManager().getClientColor(90).getRGB(), -1, true);
                    } else if (yaw <= -67.5F) {
                        ...
                        drawSidePlast(playerPos.east(5).down(), smooth, Aporia.getInstance().getThemeManager().getClientColor(90).getRGB(), 1, false);
                        ...
                        drawSidePlast(playerPos.west(4).down(), smooth, Aporia.getInstance().getThemeManager().getClientColor(90).getRGB(), 1, true);
                    } else if (yaw <= 112.5F) {
                        ...
                        drawSidePlast(playerPos.west(4).south().down(), smooth, Aporia.getInstance().getThemeManager().getClientColor(90).getRGB(), -1, false);
                    }
                }
    ...
    public void onDraw(EventRender2D e) {
        ...
        structures.forEach(cons -> {
            ...
            if (ProjectionUtil.canSee(cons.vec) && cons.anarchy == Aporia.getInstance().getServerHandler().getAnarchy() && Aporia.getInstance().getServerHandler().getWorldType().equals(cons.world)) {
                ...
                e.getContext().drawText(font, text, posX + 16 * 0.8f + 4, posY + 2.5f, Aporia.getInstance().getThemeManager().getCurrentTheme().getColor());
        ...
        serverEvents.forEach(event -> {
            ...
            if (ProjectionUtil.canSee(event.vec) && event.anarchy == Aporia.getInstance().getServerHandler().getAnarchy() && Aporia.getInstance().getServerHandler().getWorldType().equals(event.world)) {
    ...
    private void drawItemCube(BlockPos playerPos, Vec3d smooth, float size, int color) {
        ...
        boolean inBox = mc.world.getPlayers().stream().anyMatch(ent -> ent != mc.player && box.intersects(ent.getBoundingBox()) && !Aporia.getInstance().getFriendManager().isFriend(ent.getGameProfile().getName()));
        Render3DUtil.drawBox(box, inBox ? Aporia.getInstance().getThemeManager().getCurrentTheme().getColor().getRGB() : color, 3, true, true, true);
    }

    private void drawItemRadius(MatrixStack matrix, float distance, int clr) {
        float playerHalfWidth = mc.player.getWidth() / 2;
        int color = validDistance(distance) ? Aporia.getInstance().getThemeManager().getCurrentTheme().getColor().getRGB() : clr;
    ...
    private void addEvent(String name, String lvl, String owner, Vec3d vec3d, String world, int timeOpen, int timeLoot) {
        if (serverEvents.stream().noneMatch(server -> server.vec.equals(vec3d))) {
            ...
            serverEvents.add(new ServerEvent(name, lvl, owner, vec3d, world, Aporia.getInstance().getServerHandler().getAnarchy(), open, loot));
        }
    }

    private void addStructure(Item item, Vec3d vec, double time) {
        if (structures.stream().noneMatch(str -> str.vec.equals(vec))) {
            structures.add(new Structure(item, vec, Aporia.getInstance().getServerHandler().getWorldType(), Aporia.getInstance().getServerHandler().getAnarchy(), time));
        }
    }
    ...
    private boolean validDistance(float dist) {
        return dist == 0 || mc.world.getPlayers().stream().anyMatch(p -> p != mc.player && !Aporia.getInstance().getFriendManager().isFriend(p.getGameProfile().getName()) && mc.player.distanceTo(p) <= dist);
    }
