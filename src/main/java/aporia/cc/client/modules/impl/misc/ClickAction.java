...
public final class ClickAction extends Module {
    ...
    public void onKey(EventKey e) {
        if (e.isKeyDown(friendBind.getKeyCode()) && mc.crosshairTarget instanceof EntityHitResult result && result.getEntity() instanceof PlayerEntity player) {
            if (Aporia.getInstance().getFriendManager().isFriend(player.getGameProfile().getName())) Aporia.getInstance().getFriendManager().removeFriend(player.getGameProfile().getName());
            else Aporia.getInstance().getFriendManager().add(player.getGameProfile().getName());
        }
