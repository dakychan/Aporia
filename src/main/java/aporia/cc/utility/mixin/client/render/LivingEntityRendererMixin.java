package aporia.cc.utility.mixin.client.render;
...
public abstract class LivingEntityRendererMixin<T extends LivingEntity, S extends LivingEntityRenderState, M extends EntityModel<? super S>> implements IMinecraft {
    ...
    public float changeYaw(float oldValue, LivingEntity entity) {
        if (entity.equals(mc.player) &&!Aporia.getInstance().getRotationManager().isSetRotation()) {
            return MathHelper.lerpAngleDegrees(Render3DUtil.getTickDelta(),Aporia.getInstance().getRotationManager().getPreviousRotation().getYaw(),Aporia.getInstance().getRotationManager().getCurrentRotation().getYaw());
        }
        return oldValue;
    }
    ...
    public float changePitch(float oldValue, LivingEntity entity) {
        if (entity.equals(mc.player) &&!Aporia.getInstance().getRotationManager().isSetRotation()) {
            return   MathHelper.lerpAngleDegrees(Render3DUtil.getTickDelta(),Aporia.getInstance().getRotationManager().getPreviousRotation().getPitch(),Aporia.getInstance().getRotationManager().getCurrentRotation().getPitch());
