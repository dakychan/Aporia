...
public class RealmsSelectWorldTemplateScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.trailerButton = (Button)linearlayout1.addChild(Button.builder(TRAILER_BUTTON_NAME, p_89701_ -> this.onTrailer()).width(100).build());
      this.selectButton = (Button)linearlayout1.addChild(Button.builder(SELECT_BUTTON_NAME, p_89696_ -> this.selectTemplate()).width(100).build());
      linearlayout1.addChild(Button.builder(CommonComponents.GUI_CANCEL, p_89691_ -> this.onClose()).width(100).build());
      this.publisherButton = (Button)linearlayout1.addChild(Button.builder(PUBLISHER_BUTTON_NAME, p_89679_ -> this.onPublish()).width(100).build());
      this.updateButtonStates();
      this.layout.visitWidgets(p_325159_ -> {
         AbstractWidget abstractwidget = (AbstractWidget)this.addRenderableWidget(p_325159_);
      });
                     ...
                     .submit(
                        () -> {
                           if (either.right().isPresent()) {
                              ...
                              WorldTemplatePaginatedList worldtemplatepaginatedlist1 = (WorldTemplatePaginatedList)either.left().get();
                              ...
                              if (worldtemplatepaginatedlist1.templates().isEmpty()) {
                                 if (RealmsSelectWorldTemplateScreen.this.worldTemplateList.isEmpty()) {
                                    String s = I18n.get("mco.template.select.none", new Object[]{"%link"});
   ...
   class Entry extends net.minecraft.client.gui.components.ObjectSelectionList.Entry<RealmsSelectWorldTemplateScreen.Entry> {
      ...
      public void renderContent(GuiGraphics p_425202_, int p_429841_, int p_430872_, boolean p_431284_, float p_423571_) {
         ...
         if (!this.template.recommendedPlayers().isBlank()) {
            p_425202_.drawString(RealmsSelectWorldTemplateScreen.this.font, this.template.recommendedPlayers(), k, this.getContentBottom() - 4 - 5, -8355712);
         }
      }
