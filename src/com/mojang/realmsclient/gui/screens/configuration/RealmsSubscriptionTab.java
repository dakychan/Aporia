...
class RealmsSubscriptionTab extends GridLayoutTab implements RealmsConfigurationTab {
   ...
   RealmsSubscriptionTab(RealmsConfigureWorldScreen p_408051_, Minecraft p_405841_, RealmsServer p_407921_) {
      ...
      RowHelper gridlayout$rowhelper = this.layout.rowSpacing(6).createRowHelper(1);
      Font font = p_408051_.getFont();
      gridlayout$rowhelper.addChild(new StringWidget(200, 9, SUBSCRIPTION_START_LABEL, font));
      this.startDateWidget = (StringWidget)gridlayout$rowhelper.addChild(new StringWidget(200, 9, this.startDate, font));
      gridlayout$rowhelper.addChild(SpacerElement.height(2));
      this.daysLeftLabelWidget = (StringWidget)gridlayout$rowhelper.addChild(new StringWidget(200, 9, TIME_LEFT_LABEL, font));
      this.daysLeftWidget = (StringWidget)gridlayout$rowhelper.addChild(new StringWidget(200, 9, this.daysLeft, font));
      gridlayout$rowhelper.addChild(SpacerElement.height(2));
      ...
      gridlayout$rowhelper.addChild(SpacerElement.height(2));
      this.deleteButton = (Button)gridlayout$rowhelper.addChild(
         Button.builder(
      ...
      gridlayout$rowhelper.addChild(SpacerElement.height(2));
      this.subscriptionInfo = (FocusableTextWidget)gridlayout$rowhelper.addChild(
   ...
   private Component daysLeftPresentation(int p_410326_) {
      if (p_410326_ < 0 && this.serverData.expired) {
         ...
         boolean flag1 = j > 0;
         if (flag && flag1) {
            return Component.translatable("mco.configure.world.subscription.remaining.months.days", new Object[]{i, j}).withStyle(ChatFormatting.GRAY);
         } else if (flag) {
            return Component.translatable("mco.configure.world.subscription.remaining.months", new Object[]{i}).withStyle(ChatFormatting.GRAY);
         } else {
            return flag1
               ? Component.translatable("mco.configure.world.subscription.remaining.days", new Object[]{j}).withStyle(ChatFormatting.GRAY)
               : Component.empty();
         }
      }
   }
   ...
   public void updateData(RealmsServer p_407991_) {
      ...
      if (flag) {
         this.subscriptionInfo.setMessage(Component.translatable("mco.snapshot.subscription.info", new Object[]{p_407991_.parentWorldName}));
      } else {
