...
public record FireworkExplosion(FireworkExplosion.Shape shape, IntList colors, IntList fadeColors, boolean hasTrail, boolean hasTwinkle)
   ...
   public void addToTooltip(Item.TooltipContext p_328877_, Consumer<Component> p_333224_, TooltipFlag p_335960_, DataComponentGetter p_395813_) {
      p_333224_.accept(this.shape.getName().withStyle(net.minecraft.ChatFormatting.GRAY));
      this.addAdditionalTooltip(p_333224_);
   }

   public void addAdditionalTooltip(Consumer<Component> p_331797_) {
      if (!this.colors.isEmpty()) {
         p_331797_.accept(appendColors(Component.empty().withStyle(net.minecraft.ChatFormatting.GRAY), this.colors));
      }

      if (!this.fadeColors.isEmpty()) {
         p_331797_.accept(
            appendColors(
               Component.translatable("item.minecraft.firework_star.fade_to").append(CommonComponents.SPACE).withStyle(net.minecraft.ChatFormatting.GRAY),
               this.fadeColors
            )
         );
      }
      ...
      if (this.hasTrail) {
         p_331797_.accept(Component.translatable("item.minecraft.firework_star.trail").withStyle(net.minecraft.ChatFormatting.GRAY));
      }

      if (this.hasTwinkle) {
         p_331797_.accept(Component.translatable("item.minecraft.firework_star.flicker").withStyle(net.minecraft.ChatFormatting.GRAY));
      }
   }
