...
public class Climate {
   ...
   public static class ParameterList<T> {
      ...
      public T findValueBruteForce(Climate.TargetPoint p_204255_) {
         ...
         long i = ((Climate.ParameterPoint)pair.getFirst()).fitness(p_204255_);
         T t = (T)pair.getSecond();

         while (iterator.hasNext()) {
            Pair<Climate.ParameterPoint, T> pair1 = iterator.next();
            long j = ((Climate.ParameterPoint)pair1.getFirst()).fitness(p_204255_);
            if (j < i) {
               i = j;
               t = (T)pair1.getSecond();
            }
   ...
   protected static final class RTree<T> {
      ...
      public static <T> Climate.RTree<T> create(List<Pair<Climate.ParameterPoint, T>> p_186936_) {
         if (p_186936_.isEmpty()) {
            ...
            int i = ((Climate.ParameterPoint)p_186936_.get(0).getFirst()).parameterSpace().size();
            if (i != 7) {
               throw new IllegalStateException("Expecting parameter space to be 7, got " + i);
            } else {
               List<Climate.RTree.Leaf<T>> list = p_186936_.stream()
                  .map(p_186934_ -> new Climate.RTree.Leaf<>((Climate.ParameterPoint)p_186934_.getFirst(), p_186934_.getSecond()))
      ...
      private static <T> Climate.RTree.Node<T> build(int p_186921_, List<? extends Climate.RTree.Node<T>> p_186922_) {
         if (p_186922_.isEmpty()) {
            ...
            return new Climate.RTree.SubTree<>(list.stream().map(p_186919_ -> build(p_186921_, Arrays.asList(p_186919_.children))).collect(Collectors.toList()));
         }
      }
