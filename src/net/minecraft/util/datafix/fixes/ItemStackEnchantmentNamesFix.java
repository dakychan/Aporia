...
public class ItemStackEnchantmentNamesFix extends DataFix {
   private static final Int2ObjectMap<String> MAP = (Int2ObjectMap<String>)DataFixUtils.make(new Int2ObjectOpenHashMap(), p_16075_ -> {
   ...
   private Dynamic<?> fixTag(Dynamic<?> p_16073_) {
      Optional<? extends Dynamic<?>> optional = p_16073_.get("ench")
         .asStreamOpt()
         .map(
            p_16081_ -> p_16081_.map(p_145425_ -> p_145425_.set("id", p_145425_.createString((String)MAP.getOrDefault(p_145425_.get("id").asInt(0), "null"))))
         )
      ...
      return p_16073_.update(
         "StoredEnchantments",
         p_326597_ -> (Dynamic)DataFixUtils.orElse(
            p_326597_.asStreamOpt()
               .map(
                  p_145421_ -> p_145421_.map(
                     p_145423_ -> p_145423_.set("id", p_145423_.createString((String)MAP.getOrDefault(p_145423_.get("id").asInt(0), "null")))
                  )
               )
