...
public class ChunkRenamesFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("Chunk Renames; purge Level-tag", type, type1, p_199427_ -> {
         ...
         typed1 = typed1.set(DSL.remainderFinder(), mergeRemainders(p_199427_, (Dynamic)typed.get(DSL.remainderFinder())));
   ...
   private static <T> Dynamic<T> mergeRemainders(Typed<?> p_185109_, Dynamic<T> p_185110_) {
      ...
      Dynamic<T> dynamic = ((Dynamic)p_185109_.get(DSL.remainderFinder())).convert(dynamicops);
      DataResult<T> dataresult = dynamicops.getMap(p_185110_.getValue()).flatMap(p_199433_ -> dynamicops.mergeToMap(dynamic.getValue(), p_199433_));
      return dataresult.result().map(p_199436_ -> new Dynamic(dynamicops, p_199436_)).orElse(p_185110_);
   }
}
