...
public class EntityUUIDFix extends AbstractUUIDFix {
   ...
   private static Dynamic<?> updateFox(Dynamic<?> p_15758_) {
      Optional<Dynamic<?>> optional = p_15758_.get("TrustedUUIDs")
         .result()
         .map(p_15748_ -> p_15758_.createList(p_15748_.asStream().map(p_145341_ -> (Dynamic)createUUIDFromML((Dynamic<?>)p_145341_).orElseGet(() -> {
            LOGGER.warn("Trusted contained invalid data.");
            return (Dynamic<?>)p_145341_;
         }))));
      return (Dynamic<?>)DataFixUtils.orElse(optional.map(p_15743_ -> p_15758_.remove("TrustedUUIDs").set("Trusted", p_15743_)), p_15758_);
   }
               ...
               .map(
                  p_145337_ -> p_145337_.update(
                     ...
                     p_145335_ -> p_145337_.createList(
                        p_145335_.asStream()
                           .map(p_145339_ -> (Dynamic)replaceUUIDLeastMost((Dynamic<?>)p_145339_, "UUID", "UUID").orElse((Dynamic<?>)p_145339_))
                     )
                  )
               )
         )
