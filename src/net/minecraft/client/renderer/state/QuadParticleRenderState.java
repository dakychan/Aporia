...
public class QuadParticleRenderState implements SubmitNodeCollector.ParticleGroupRenderer, ParticleGroupRenderState {
   ...
   public QuadParticleRenderState.@Nullable PreparedBuffers prepare(ParticleFeatureRenderer.ParticleBufferCache p_429684_) {
      ...
      ByteBufferBuilder bytebufferbuilder = ByteBufferBuilder.exactlySized(i * DefaultVertexFormat.PARTICLE.getVertexSize());

      QuadParticleRenderState.PreparedBuffers var10;
      label56: {
         Object object;
         try {
            BufferBuilder bufferbuilder = new BufferBuilder(bytebufferbuilder, Mode.QUADS, DefaultVertexFormat.PARTICLE);
            ...
            if (meshdata != null) {
               ...
               RenderSystem.getSequentialBuffer(Mode.QUADS).getBuffer(meshdata.drawState().indexCount());
               GpuBufferSlice gpubufferslice = RenderSystem.getDynamicUniforms()
                  .writeTransform(RenderSystem.getModelViewMatrix(), new Vector4f(1.0F, 1.0F, 1.0F, 1.0F), new Vector3f(), new Matrix4f());
               var10 = new QuadParticleRenderState.PreparedBuffers(meshdata.drawState().indexCount(), gpubufferslice, map);
               break label56;
            }
