...
public class CloudRenderer extends SimplePreparableReloadListener<Optional<CloudRenderer.TextureData>> implements AutoCloseable {
   ...
   protected Optional<CloudRenderer.TextureData> prepare(ResourceManager p_361257_, ProfilerFiller p_362196_) {
      try {
         Optional optional;
         try (InputStream inputstream = p_361257_.open(TEXTURE_LOCATION)) {
         ...
         LOGGER.error("Failed to load cloud texture", var20);
   ...
   public void render(int p_369834_, CloudStatus p_363277_, float p_367079_, Vec3 p_367264_, long p_455582_, float p_364211_) {
      if (this.texture != null) {
         ...
         if (this.needsRebuild
            ...
            this.utb.rotate();
            MappedView gpubuffer$mappedview = RenderSystem.getDevice().createCommandEncoder().mapBuffer(this.utb.currentBuffer(), false, true);
         ...
         if (this.quadCount != 0) {
            MappedView gpubuffer$mappedview1 = RenderSystem.getDevice().createCommandEncoder().mapBuffer(this.ubo.currentBuffer(), false, true);
            ...
            try {
               ...
               if (gpubuffer$mappedview1 != null) {
                  try {
                     gpubuffer$mappedview1.close();
