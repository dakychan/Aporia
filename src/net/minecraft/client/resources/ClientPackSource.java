...
public class ClientPackSource extends BuiltInPackSource {
    ...
    private static VanillaPackResources createVanillaPackSource(final Path externalAssetRoot) {
        VanillaPackResourcesBuilder builder = new VanillaPackResourcesBuilder()
            ...
            .exposeNamespace("minecraft", "realms", "aporia")
            .applyDevelopmentConfig()
            .pushJarResources()
            .pushClasspathResources(PackType.CLIENT_RESOURCES, ClientPackSource.class)
            .pushAssetPath(PackType.CLIENT_RESOURCES, externalAssetRoot);
        Path aprAssets = java.nio.file.Paths.get(System.getProperty("user.home"), ".apr", ".assets");
        if (java.nio.file.Files.exists(aprAssets)) {
            builder.pushAssetPath(PackType.CLIENT_RESOURCES, aprAssets);
        }
        Path ccAssetsDir = so.aporia.utils.files.FilesManager.ROOT.resolve(".assets");
        if (java.nio.file.Files.exists(ccAssetsDir)) {
            builder.pushAssetPath(PackType.CLIENT_RESOURCES, ccAssetsDir);
        }
        return builder.build(VANILLA_PACK_INFO);
    }
