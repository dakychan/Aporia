...
public class ChatComponent {
   ...
   public String getMessageAt(int mouseX, int mouseY) {
      if (this.isChatHidden()) {
         return null;
      } else {
         double scale = this.getScale();
         int scaledX = (int)(mouseX / scale) - 4;
         ...
         if (scaledX < 0) {
            ...
            int yFromBottom = chatBottom - scaledY;
            if (yFromBottom >= 0 && yFromBottom <= chatHeight) {
               int lineIndex = yFromBottom / lineHeight;
               if (lineIndex >= 0 && lineIndex < linesPerPage) {
                  int messageIndex = lineIndex + this.chatScrollbarPos;
                  if (messageIndex >= 0 && messageIndex < this.trimmedMessages.size()) {
                     GuiMessage.Line line = this.trimmedMessages.get(messageIndex);

                     for (GuiMessage message : this.allMessages) {
                        if (message.addedTime() == line.addedTime()) {
   ...
   private void render(final ChatComponent.ChatGraphicsAccess p_454435_, int p_283491_, int p_282406_, boolean p_328818_) {
      if (!this.isChatHidden()) {
         ...
         if (i > 0) {
            ...
            if (l1 > 0L) {
               p_454435_.fill(-2, k, j + 4, k + 9, ARGB.black(f2));
            }
   ...
   public void addMessage(Component p_241484_, @Nullable MessageSignature p_241323_, @Nullable GuiMessageTag p_241297_) {
      ...
      if (!playerName.isEmpty() && messageText.toLowerCase().contains(playerName.toLowerCase())) {
         Manager.getInstance().showNotification(Messages.chatMention(), NotificationType.MENTION);
      }
   ...
   private void logChatMessage(GuiMessage p_328461_) {
      ...
      String s1 = net.minecraft.Optionull.map(p_328461_.tag(), GuiMessageTag::logTag);
      if (s1 != null) {
   ...
   public void openScreen(ChatComponent.ChatMethod p_427648_, ChatScreen.ChatConstructor<?> p_427995_) {
      this.minecraft.setScreen(this.createScreen(p_427648_, (ChatScreen.ChatConstructor<Screen>)p_427995_));
   }
   ...
   public static enum ChatMethod {
      ...
      private ChatMethod(final String p_427929_) {
