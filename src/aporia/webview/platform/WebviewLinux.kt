...
/** Linux WebView stub — replace with WebKitGTK JNA bindings when available. */
class WebviewLinux(url: String, width: Int, height: Int) : Webview(url, width, height) {
    ...
    private fun fill() {
        ...
        for (y in 0 until height) for (x in 0 until width) {
            val c = (x / 40 + y / 40) % 2 == 0
            b.put(if (c) 0x40.toByte() else 0x00.toByte())
            b.put(if (c) 0x80.toByte() else 0x40.toByte())
            b.put(if (c) 0x00.toByte() else 0x20.toByte())
            b.put(0xFF.toByte())
        }
    }
