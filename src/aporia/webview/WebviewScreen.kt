...
class WebviewScreen(url: String) : Screen(Component.literal("Webview")) {
    ...
    private val tabDir: Path = FilesManager.ROOT.resolve(".webview/tabs")
    ...
    init {
        Minecraft.getInstance().textureManager.register(WEBVIEW_TEX_ID, webviewTex)
        val w = Minecraft.getInstance().window.guiScaledWidth
        val h = Minecraft.getInstance().window.guiScaledHeight - toolbarH
