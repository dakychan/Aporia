...
layout(std140) uniform BlurData {
    ...
    float Saturation; // 0=grayscale, 0.5=normal, 1.0=enhanced — applied on H pass only
    float _pad0;
    float _pad1;
    float _pad2;
};
...
void main() {
    ...
    // 9-tap 1D gaussian weights (precomputed for sigma~1.5)
    float w[5];
    w[0] = 0.2270270270;
    w[1] = 0.1945945946;
    w[2] = 0.1216216216;
    ...
    // apply saturation on horizontal pass only (avoid double-applying)
    if (Direction < 0.5) {
