...
void main() {
    ...
    // Применяем blur в ОБОИХ направлениях за один проход
    vec4 color = vec4(0.0);
    
    for (int y = -samples; y <= samples; ++y) {
        for (int x = -samples; x <= samples; ++x) {
            vec2 offset = vec2(float(x) * uTexelSize.x, float(y) * uTexelSize.y);
            float weight = weights[abs(x)] * weights[abs(y)] * normalization * normalization;
            color += texture(uSource, vUv + offset) * weight;
        }
    }
