...
public class AporiaChatScreen extends ChatScreen {
    ...
    /**
     * Конфигурация окна чата: позиция, размер, преффикс/суффикс, фильтр, скролл
     * и хранилище строк, заполняемое {@link WinMgr#route}.
     * <p>
    ...
    public static final class WinCfg {
        ...
        /** Анимация для названия окна / Window name animation. */
    ...
    public boolean mouseScrolled(double mx, double my, double dx, double dy) {
         ...
         *  Check scroll for the active window. */
        WinCfg c = cfg();
        int bx = boxX(), by = boxY(), bw = boxW(), bh = boxH();
        if (mx >= bx && mx <= bx+bw && my >= by && my <= by+bh) {
            int delta = (dy > 0 ? 1 : -1);
         ...
         *  Check scroll for inactive windows. */
        for (int i = 0; i < WinMgr.I.wins.size(); i++) {
            if (i == WinMgr.I.active) continue;
            WinCfg w = WinMgr.I.wins.get(i);
            resolvePosition(w, i);
     ...
     */
     ...
     public void handleChatInput(String msg, boolean addToHistory) {
         ...
         /** Обработка команд Aporia (напр. .help, .prefix).
