...
object BlurRenderer {
    ...
    fun init() {
        ...
        kawaseUpPipeline = RenderPipeline.builder()
            ...
            .withBindGroupLayout(BindGroupLayout.builder().withUniform("KawaseData", UniformType.UNIFORM_BUFFER).build())
    ...
    fun ensureBlurTarget(mc: Minecraft) {
        ...
        blurTarget = TextureTarget("aporia_blur_final", mainW, mainH, false, GpuFormat.RGBA8_UNORM)
        var cw = mainW; var ch = mainH
        for (i in kawaseDownTargets.indices) {
            cw = Math.max(1, cw / 2); ch = Math.max(1, ch / 2)
            kawaseDownTargets[i] = TextureTarget("aporia_blur_down_$i", cw, ch, false, GpuFormat.RGBA8_UNORM)
        }
    }
    ...
    fun ensureGuiBlurTarget(mc: Minecraft) {
        ...
        guiBlurTarget = TextureTarget("aporia_gui_blur", mainW, mainH, false, GpuFormat.RGBA8_UNORM)
    ...
    fun ensurePrePlayerBlurTarget(mc: Minecraft) {
        ...
        prePlayerBlurTarget = TextureTarget("aporia_pre_player_blur", mainW, mainH, false, GpuFormat.RGBA8_UNORM)
    ...
    @JvmStatic fun prepareGuiBlur(mc: Minecraft, strength: Float, saturation: Float) {
        ...
        for (i in maxSteps - 1 downTo 0) {
            ...
            val pass = encoder.createRenderPass({ -> "aporia:gui_kawase_d_$i" }, currentDst.colorTextureView!!, Optional.empty<Vector4fc>())
            pass.use {
                ...
                it.setVertexBuffer(0, blurQuadVbo.slice(0L, blurQuadVbo.size()))
                it.draw(6, 1, 0, 0)
            }
    ...
    @JvmStatic fun prepareBlur(mc: Minecraft, strength: Float, saturation: Float) {
        ...
        for (i in maxSteps - 1 downTo 0) {
            ...
            val offset = 0.5f + (maxSteps - 1 - i) * 0.25f
            fillBlurBB(currentDst, offset); encoder.writeToBuffer(blurUbo.slice(), cachedBlurBB!!)
            val pass = encoder.createRenderPass({ -> "aporia:kawase_d_$i" }, currentDst.colorTextureView!!, Optional.empty<Vector4fc>())
            pass.use {
                it.setPipeline(kawaseUpPipeline)
                it.bindTexture("InputTexture", currentSrc.colorTextureView!!,
                    RenderSystem.getSamplerCache().getClampToEdge(FilterMode.LINEAR))
        ...
        for (i in maxSteps - 1 downTo 0) {
            ...
            val offset = 0.5f
            fillBlurBB(currentDst, offset); encoder.writeToBuffer(blurUbo.slice(), cachedBlurBB!!)
            val pass = encoder.createRenderPass({ -> "aporia:kawase_u_$i" }, currentDst.colorTextureView!!, Optional.empty<Vector4fc>())
            pass.use {
                it.setPipeline(kawaseUpPipeline)
                it.bindTexture("InputTexture", currentSrc.colorTextureView!!,
                    RenderSystem.getSamplerCache().getClampToEdge(FilterMode.LINEAR))
    ...
    @JvmStatic fun prepareBlurForPlayer(mc: Minecraft, strength: Float, saturation: Float) {
        ...
        val device = RenderSystem.getDevice()
        val encoder = device.createCommandEncoder()
        val pass = encoder.createRenderPass({ -> "aporia:player_blit" }, prePlayerBlurTarget!!.colorTextureView!!, Optional.empty<Vector4fc>())
        pass.use {
    ...
    @JvmStatic fun close() {
        cleanup()
        if (::blurQuadVbo.isInitialized) blurQuadVbo.close()
        if (::blurUbo.isInitialized) blurUbo.close()
    }
}
