...
public class FontPipeline {
    ...
    /** Projection matrix for vanilla font fallback */
    private CachedOrthoProjectionMatrixBuffer fallbackProjection;
    ...
    private void ensureInitialized() {
        ...
        fallbackProjection = new CachedOrthoProjectionMatrixBuffer("aporia:font_fallback", -1000f, 1000f, true);
        initialized = true;
    }
    ...
    private void appendGlyphs(FontAtlas atlas, String text, float startX, float startY,
        ...
        while (i < text.length()) {
            ...
            if (glyph == null) {
                FontAtlas savedAtlas = currentAtlas;
                float savedOutlineW = currentOutlineWidth;
                int savedOutlineC = currentOutlineColor;

                flush();

                float size = scale * atlas.getFontSize();
                renderVanillaFallback(cp, cursorX, cursorY, currentColor, size);

                currentAtlas = savedAtlas;
                currentOutlineWidth = savedOutlineW;
                currentOutlineColor = savedOutlineC;

                var mc = Minecraft.getInstance();
                if (mc.font != null) {
                    String charStr = new String(Character.toChars(cp));
                    cursorX += mc.font.width(charStr) * (size / mc.font.lineHeight);
                } else {
                    cursorX += atlas.getFontSize() * scale * 0.5f;
                }
                i += cc; continue;
            }
    ...
    /**
     * Renders a single character using the vanilla Minecraft font (TTF/bitmap fallback).
     * Used when the MSDF atlas does not have the glyph.
     * <p>
     * Рендерит один символ через ванильный Minecraft шрифт (TTF/bitmap fallback).
     * Используется когда в MSDF атласе нет глифа.
     */
    private void renderVanillaFallback(int cp, float x, float y, int color, float size) {
        var mc = Minecraft.getInstance();
        var font = mc.font;
        if (font == null) return;
        String str = new String(Character.toChars(cp));

        float vanillaScale = size / font.lineHeight;
        ...
        RenderSystem.setProjectionMatrix(fallbackProjection.getBuffer(sw, sh), ProjectionType.ORTHOGRAPHIC);

        var pose = new Matrix4f().translation(x, y + size * 0.12f, 0).scale(vanillaScale);
        font.drawInBatch(str, 0, 0, color, false, pose,
    ...
    public void close() {
        ...
        if (fallbackProjection != null) { fallbackProjection.close(); fallbackProjection = null; }
        initialized = false;
    }
